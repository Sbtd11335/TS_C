...
推奨性能
メモリ:8GB以上
CPU:Core i7-1165G7、i7-12700Hでの動作確認済み、Core i5以上を推奨
...
F12:スクリーンショット
SelectSong
F4:オート(OFF,1P_AUTO,2P_AUTO,AUTO)
F6:1P強制譜面分岐(OFF,普通譜面、玄人譜面、達人譜面)
F7:1P強制譜面分岐(OFF,普通譜面、玄人譜面、達人譜面)
F8:等速設定(OFF,1P等速、2P等速、両プレイヤー等速)
...
//Game
UPCheck:ソフト起動毎にアップデートの確認を行います。(FALSE,TRUE)
...
GAUGECOMBO:魂ゲージが100%になる時のコンボ数の指定(int)
SONGTAG:選曲時に曲名の左側に画像を置く(int)
BGMOVIE:背景を指定した動画にすり替える(str)
SONGID:譜面のスコアなどのデータの同期(str)
...
//それより下の値の設定
自作スキンに使うどんちゃんの画像の枚数、拡大率、加算座標を指定できます。
...
#E 玄人譜面
#M 達人譜面
#LINECOLOR (int)小節線の色の変更
#LINELOOP (int,Scroll0,Scroll1)指定した数だけ小節線を置く
